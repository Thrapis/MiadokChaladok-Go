...
import { IFilterForm, IFilterLists } from 'shared/api/catalog'
...
type Props = {
    ...
    catalogFilters: IFilterLists | undefined
}
                                    ...
                                    render={
                                        ({ field: { onChange, name, value } }) => {
                                            const options = catalogFilters?.categories?.map(
                                                category => ({ 'value': `${category.id}`, 'title': category.name })
                                            ) || []
                                            return (
                                                <CheckboxGroup
                                                    size='small'
                                    ...
                                    render={
                                        ({ field: { onChange, name, value } }) => {
                                            const options = catalogFilters?.tastes?.map(
                                                taste => ({ 'value': `${taste.id}`, 'title': taste.name })
                                            ) || []
                                            return (
                                                <CheckboxGroup
                                                    size='small'
                                    ...
                                    render={
                                        ({ field: { onChange, name, value } }) => {
                                            const options = catalogFilters?.shipmentMethods?.map(
                                                method => ({ 'value': `${method.id}`, 'title': method.name })
                                            ) || []
                                            return (
                                                <CheckboxGroup
                                                    size='small'
